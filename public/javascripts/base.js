...
function verifyEPCID(epcid)
{
	if (null == epcid || undefined == epcid ||
		12 != epcid.length ||
		53 < parseInt(epcid.substr(2,2)) || parseInt(epcid.substr(2,2)) < 1 ||	// week number
		parseInt(epcid.substr(4,2)) < 17											// since 2017
	) {
		return false;
	}

	var arr = ['NJ', 'SH', 'BJ', 'HZ', 'CD'];
	for (var i in arr) {
		if (epcid.substr(0,2) == arr[i]) {
			return 'E' == epcid.substr(6,1) || 'Z' == epcid.substr(6,1);
		}
	}
	return false;
}